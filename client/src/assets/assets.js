...
// Dummy reviews — keyed by product _id
const reviewPool = [
  { name: 'Sarah M.', rating: 5, date: 'Jul 10, 2025', comment: 'Absolutely fresh and delivered super fast! Will order again.' },
  { name: 'Raj K.', rating: 4, date: 'Jul 8, 2025', comment: 'Good quality, packaging was neat. Slightly smaller than expected.' },
  { name: 'Priya S.', rating: 5, date: 'Jul 5, 2025', comment: 'Best quality I have found online. Taste is amazing.' },
  { name: 'Ahmed L.', rating: 4, date: 'Jun 30, 2025', comment: 'Very fresh, arrived on time. Happy with the purchase.' },
  { name: 'Emily R.', rating: 3, date: 'Jun 28, 2025', comment: 'Decent product but delivery took a bit longer than usual.' },
  { name: 'Vikram T.', rating: 5, date: 'Jun 25, 2025', comment: 'Superb freshness! Exactly as described. Highly recommended.' },
  { name: 'Fatima N.', rating: 4, date: 'Jun 22, 2025', comment: 'Great value for money. Fresh and well packed.' },
  { name: 'James O.', rating: 5, date: 'Jun 20, 2025', comment: 'Farm fresh quality. My family loved it. Perfect purchase.' },
];

export const dummyReviews = dummyProducts.reduce((acc, product, idx) => {
  // Each product gets 3–5 reviews cycling through the pool
  const count = (idx % 3) + 3;
  acc[product._id] = Array.from({ length: count }, (_, i) => ({
    ...reviewPool[(idx + i) % reviewPool.length],
    id: `${product._id}-r${i}`,
  }));
  return acc;
}, {});
